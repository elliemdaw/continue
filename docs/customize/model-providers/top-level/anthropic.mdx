...
Anthropic models are also available through Azure AI Foundry. To use Azure-hosted Anthropic endpoints, configure the `apiBase` parameter with your Azure resource URL:
...
<Tabs>
  ...
  models:
    - name: <MODEL_NAME>
      provider: anthropic
      model: <MODEL_ID>
      apiBase: https://<YOUR_RESOURCE>.services.ai.azure.com/models
...
<Info>
  Azure endpoints use the `api-key` header format instead of the standard `x-api-key` header. Continue automatically detects Azure endpoints and applies the correct authentication method.
</Info>
